//+------------------------------------------------------------------+
//|                                 BlackCrows_WhiteSoldiers_CCI.mqh |
//|                                                       Dylan Ding |
...
class BlackCrowsWhiteSoldiersCCI {
private:
//--- Input parameters
int  InpAverBodyPeriod=12;          // period for calculating average candlestick size
int  InpPeriodCCI     =37;          // CCI period
ENUM_APPLIED_PRICE InpPrice=PRICE_CLOSE; // price type

//--- trade parameters
uint InpDuration = 10;              // position holding time in bars
uint InpSL       = 200;             // Stop Loss in points
uint InpTP       = 200;             // Take Profit in points
uint InpSlippage = 10;              // slippage in points
//--- money management parameters
double InpLot=0.1;                  // lot
//--- Expert ID
long InpMagicNumber=120100;         // Magic Number
...
double ExtAvgBody;                  // average candlestick body size
...
//+------------------------------------------------------------------+
void CloseByTime(datetime next_bar_time)
  {
//--- if there are no positions opened by our EA
   if(PositionExist(ExtSignalOpen)==0)
      return;
//--- calculate the closing limit
   datetime max_date=iTime(_Symbol, _Period, InpDuration);
   if(max_date==0)
     {
      //--- failed to get max date, retry on the next tick
      return;
     }
